...
FILE_NAME_TAG="demp"
DATA_URL="S3/eictest/EPIC/EVGEN/EXCLUSIVE/DEMP/5on100/eic_DEMPGen_5on100_ip6_pi+_1B_1.hepmc"
...
if [[ -n "${DO_ANALYSIS}" || -n "${DO_ALL}" ]] ; then
  ...
  mkdir -p results/demp

  # here you can add as many scripts as you want.
  root -b -q "benchmarks/demp/analysis/demp_tests.C(\"${JUGGLER_REC_FILE}\")"
  if [[ "$?" -ne "0" ]] ; then
