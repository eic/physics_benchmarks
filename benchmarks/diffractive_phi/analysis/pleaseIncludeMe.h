...
auto giveme_t_A = [](std::vector<ROOT::Math::PxPyPzMVector> vm, 
  ...
  for(auto& i2: scatElec){
    for(auto& i1: vm){
      ...
      double e_pt_res=gRandom->Gaus(0.0,0.0068);//set this number by full simulations for Q2>15
      double e_pt = eOut.Pt()*(1.+e_pt_res);
      eOut.SetPerp(e_pt);

      double vm_pt_res=gRandom->Gaus(0.0,0.0065);//set this number by full simulations for Q2>15
