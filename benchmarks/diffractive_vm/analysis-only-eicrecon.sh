...
root -b -q "benchmarks/diffractive_vm/analysis/diffractive_vm_analysis_eicrecon.cxx+(\"${CONFIG}\",${VM_TYPE_TAG},${MC_TYPE_TAG})"
if [[ "$?" -ne "0" ]] ; then
  echo "ERROR running rec_diffractive_vm_analysis_eicrecon script"
  exit 1
fi
