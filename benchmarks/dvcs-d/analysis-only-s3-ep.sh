...
echo "Running the dvcs-ep analysis"
...
EOF
#cat ${CONFIG}
root -b -q "benchmarks/dvcs-d/analysis/dvcs_ep_analysis.cxx+(\"${CONFIG}\")"
if [[ "$?" -ne "0" ]] ; then
  echo "ERROR running rec_dvcs-ep_analysis script"
  exit 1
fi

echo "DVCS-ep analysis complete"
